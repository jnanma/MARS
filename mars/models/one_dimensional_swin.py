...
from mars.models.position_emb import get_positional_embed, default
...
class CirculateSwinAttention(nn.Module):
    def __init__(self, dim, window_size,
                 heads, dim_key, dropout, 
                 dw_conv, reduce=False, squeeze=False):
        super().__init__()
        self.redeuce = reduce
        self.half_window_size = window_size // 2
        self.regular_swin = CirculateSwinAttentionHelp(
                            dim=dim,
                            window_size=window_size,
                            heads=heads,
                            dim_key=dim_key, #attn_dim_key
                            dropout=dropout,
                            dw_conv=dw_conv
                        )
        self.shift_swin = CirculateSwinAttentionHelp(
                            dim=dim,
                            window_size=window_size,
                            heads=heads,
                            dim_key=dim_key,
    ...
    def forward(self, x):
        ...
        x = torch.roll(x, self.half_window_size, dims=1)
        if self.redeuce:
            x0 = x[:, 0::2, :] #选取了x的每一列中的偶数索引行
            x1 = x[:, 1::2, :] #选取了x的每一列中的奇数索引行
            if x1.shape[1] < x0.shape[1]:
                to_pad = x0[:, -1, :].unsqueeze(1)
                x1 = torch.cat([x1, to_pad], 1)
            x = torch.cat([x0, x1], -1)  #合并两行特征，维度减半
            if self.squeeze:
                x = self.linear(x) #redeuce且squeeze，维度减半
            # else:
            #     x = self.linear(x)
...
class SwinAttentionHelp(nn.Module):
    ...
    def forward(self, x):
        ...
        original_n = n
        device = x.device
        remainder = n % self.window_size #取余
        ...
        if self.padding_mode is False:
            ...
            if needs_padding:
                ...
                n += padding_size
        qkv = self.to_qkv(x) #output shape: [b,n,h*d*k]
        ...
        )  #output: [0, self.window_size, ...,self.window_size*n, ...(能整除的话n-self.window_size+1)]
